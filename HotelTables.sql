...
CREATE TABLE Facility ( Facility_Id int IDENTITY (1,1) NOT NULL PRIMARY KEY, Name VARCHAR(30) NOT NULL);

CREATE TABLE HotelFacility (HotelFacility_Id int IDENTITY (1,1) NOT NULL PRIMARY KEY, Hotel_No int NOT NULL, Facility_Id int NOT NULL, FOREIGN KEY (Hotel_No) REFERENCES Hotel (Hotel_No), FOREIGN KEY (Facility_Id) REFERENCES Facility (Facility_Id))